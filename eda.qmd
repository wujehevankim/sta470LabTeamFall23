...
```{r}
t.test(hospital_A_surgery$recalibrated_time_minutes, hospital_B_surgery$recalibrated_time_minutes, var.equal=FALSE)
```

```{r}
Model_with_random_effects <- glmer(complication == "Yes" ~ age + recalibrated_time_minutes + (1 | hospital), data = surgery_df, family = binomial)

summary(Model_with_random_effects)
```
